import { bigint, index, jsonb, pgEnum, pgTable, timestamp, uuid, varchar } from 'drizzle-orm/pg-core';

const paymentStatusEnum = pgEnum('payment_status', ['initiated', 'processing', 'succeeded', 'failed', 'canceled']);

const paymentProviderEnum = pgEnum('payment_provider', ['stripe']);

const postgresPaymentSchema = pgTable(
    'payments',
    {
    ...
    (table) => ({
        providerProviderPaymentIdIdx: index('payments_provider_provider_payment_id_idx').on(
            table.provider,
